...
def arrowLogic(ELEMENTS):
    ...
    if ELEMENTS["event"] == KEYDOWN and ELEMENTS["keysPressed"][K_SPACE]:
        ...
        match ARROW.option:
            case "SINGLE PLAYER":
                game_flag = "GAMEPLAY"
                ELEMENTS["mode"] = "SINGLE PLAYER"
            case "TWO PLAYER":
                game_flag = "GAMEPLAY"
                ELEMENTS["mode"] = "TWO PLAYER"
            case "EXIT":
                game_flag = "EXIT"
...
def mainMenu(CONTAINER):
    ...
    ON_SCREEN.add([CONTAINER["ALL"].sprites()[8],
                   CONTAINER["ALL"].sprites()[12],
                   CONTAINER["ALL"].sprites()[13],
